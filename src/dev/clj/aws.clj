...
(defn create-ec2-instance-security-group []
      ...
      :user-data (encode-base64
                   (str/join "\n"
                             ["#!/bin/bash"
                              ...
                              (str
                                "aws ec2 associate-address --region eu-central-1 --instance-id ${INSTANCE_ID} --allocation-id "
                                allocation-id
                                " --allow-reassociation")
...
(defn register-backend-task-definition [version]
  (:task-definition
    (ecs/register-task-definition
      ...
      [{:cpu                1024
        :essential          true
        :image              (str (docker-repository-uri) ":" version)
...
(defn authorize-deployments-by-circleci-user []
  (iam/put-user-policy
    :user-name circleci-user-name
    :policy-name "circle-deploy"
    :policy-document (json/encode {"Version"   "2012-10-17"
                                   "Statement" [{"Effect"   "Allow",
